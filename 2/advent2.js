...
const solution1 = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    array.push([
      arr[i].match(reMin)[0],
      arr[i].match(reMax)[0].replace("-", ""),
      arr[i].match(reLet)[0],
      arr[i].match(rePwd)[0],
    ]);

    let re = new RegExp(array[i][2], "g");
    if (array[i][3].match(re)) {
      if (
        array[i][3].match(re).length >= array[i][0] &&
        array[i][3].match(re).length <= array[i][1]
      ) {
        total += 1;
      }
    }
...
const solution2 = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let pwdArray = array[i][3].split("");
    let oneMatch = array[i][0] - 1;
    let twoMatch = array[i][1] - 1;
    let letMatch = array[i][2];
    if (pwdArray[oneMatch] == letMatch || pwdArray[twoMatch] == letMatch) {
      if (!(pwdArray[oneMatch] == letMatch && pwdArray[twoMatch] == letMatch)) {
        total += 1;
      }
    }
...
console.log("The number of valid passwords is ", solution1(passwordList));
console.log(
  "The number of valid passwords for the second solution is ",
  solution2(passwordList)
);
